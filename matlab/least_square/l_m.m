function [a, b, c, d, sum_sq, t, iter] = l_m(f, data)
...
while true
    ...
    while true
        %% Compute direction
        p = -inv(J'*J + lambda*eye(length(J(1,:))))*J'*r;
        %% Calculate new guess
        B = prev_B - p;
        sum = 0;
        for i=1:m
            arg = [B', [x_dat(i), y_dat(i), z_dat(i)]];
            sum = sum + subs(f, {'a', 'b', 'c', 'd', 'x', 'y', 'z'}, arg)^2;
        end
        sum = eval(sum);
        %% When increased, continue loop
        if sum > prev_sum
            lambda = lambda*10;
        %% When decreased, use lambda and newely derived assumptions
        else
            lambda = lambda/10;
            break;
        end
